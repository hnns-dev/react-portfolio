...
function ProjectCard({
  ...
  return (
    <div className="card bg-stone-400 bg-opacity-20 backdrop-blur-lg rounded-xl overflow-hidden shadow-lg">
      <div
        className="h-48 bg-cover bg-center"
        style={{ backgroundImage: `url(${imageUrl})` }}
      ></div>
      <div className="p-6">
        <p className="text-xl font-bold text-amber-500 mb-2">{title}</p>
        <p className="text-amber-50 mb-4 font-extralight">{description}</p>
        <div className="flex justify-between">
          <div className="flex items-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 640 512"
              className="w-5 h-5 mr-2 fill-current text-amber-400"
            >
            ...
            <a
              className="link text-amber-400 hover:text-amber-600"
              href={previewLink}
              target="_blank"
